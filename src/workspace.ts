...
async function tryClaudeCliGeneration(prompt: string, wsDir: string): Promise<boolean> {
  try {
    ...
    try {
      ...
      let agentIoModule: unknown = null;
      try {
      ...
      if (agentIoModule && 
          ...
          typeof (agentIoModule as { createStreamRenderer: unknown }).createStreamRenderer === 'function') {
        const moduleWithRenderer = agentIoModule as { createStreamRenderer: (options: { onText: (text: string) => void }) => { push: (data: string) => void } | null };
